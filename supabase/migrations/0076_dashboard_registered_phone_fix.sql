-- =============================================================================
-- 0076_dashboard_registered_phone_fix.sql
-- المَحجّة البَيْضَاء — fix "المسجلون" tile on لوحة المعلومات (admin_dashboard_stats).
--
-- Same bug as 0075, different function: 0026 defined registered_users as
-- `u.email is not null`, so the dashboard tile showed 4 while the real count
-- of registered (non-guest) students is much higher — registration here
-- accepts phone OR email (phone required, email optional), so an email-only
-- check misses every phone-only registration, which is most of them.
--
-- Fix: use auth.users.is_anonymous = false, same signal as admin_user_list
-- (migration 0075). Only the registered_users line changes; everything else
-- in admin_dashboard_stats is unchanged from 0026.
--
-- Append-only — 0001–0075 are never edited. Idempotent.
-- =============================================================================
...
-- No signature change (same args, still `()`) — grant is unaffected by
-- `create or replace` on an unchanged signature, but re-assert it anyway per
-- 0039 hygiene convention (harmless no-op if already correct).
